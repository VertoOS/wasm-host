...
session protocol loop, but it is not the full native app-server engine.
...
| Browser app-server protocol | Codex browser demo speaks app-server JSON-RPC over WebSocket, but its server spawns native `codex app-server`. The browser adapter now has a deterministic browser-owned app-server fixture covering initialize, account read, device login cancel, thread start, mocked turn completion, turn interrupt, notification opt-out, bounded notifications, and unsupported capability errors. | Browser-hosted app-server/session runtime that does not require a native child process. | [#118](https://github.com/VertoOS/wasm-host/issues/118) |
...
| M9: browser-hosted app-server | The first deterministic browser app-server fixture runs the core JSON-RPC protocol subset in the browser host without spawning native `codex app-server`; full session state, real auth/provider traffic, persistence, and UI WebSocket replacement remain later. | Browser e2e using app-server JSON-RPC over the host runtime. | [#118](https://github.com/VertoOS/wasm-host/issues/118), [#117](https://github.com/VertoOS/wasm-host/issues/117) |
