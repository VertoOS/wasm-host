...
| Workspace | Raw version smoke has read-only package files and volatile `/tmp`. A separate browser workspace store now supports in-memory operations plus whole-snapshot IndexedDB persistence, but it is not wired into WASI, the app-server path, or Codex edit turns yet. | Persistent browser workspace with clear mount semantics, snapshots, import/export, and file edit turn coverage. | [#113](https://github.com/VertoOS/wasm-host/issues/113), [#1](https://github.com/VertoOS/wasm-host/issues/1) |
