...
| Shell, git, and tools | Native runner has explicit host-command bridges for development. The browser command worker can catalog loaded package commands, resolve explicit `packageId: null` runs through PATH, invoke loaded child packaged commands with cwd/env/stdin/stdout/stderr/timeout propagation, route raw WASI worker child-command requests through an internal RPC, support `wasix_32v1.proc_exec`/`proc_exec2`/`proc_exec3` for packaged command replacement with env/PATH overlays, project catalog command shims for guest shell PATH probes, and run deterministic packaged tool fixtures with terminal transcript capture. The first non-interactive Bash/coreutils WebC smoke now passes in a real browser; real git, broad shell behavior, process spawn, general blocking join, full fork/store cloning, and native process spawning remain later. | Codex tool calls run sandboxed packaged commands with stdio, cwd, env, timeout, cancellation, and child command semantics. | [#212](https://github.com/VertoOS/wasm-host/issues/212), [#181](https://github.com/VertoOS/wasm-host/issues/181), [#179](https://github.com/VertoOS/wasm-host/issues/179), [#177](https://github.com/VertoOS/wasm-host/issues/177), [#175](https://github.com/VertoOS/wasm-host/issues/175), [#114](https://github.com/VertoOS/wasm-host/issues/114), [#6](https://github.com/VertoOS/wasm-host/issues/6) |
...
| End-to-end parity | Current e2e proves `codex --version`, request building, a mocked model turn, workspace edit persistence, a browser app-server protocol fixture, a packaged tool fixture through the terminal transcript path, and the first non-interactive Bash/coreutils WebC smoke. The page-level smoke result reports these as named stages so partial browser support is visible in CI. | Add staged Codex browser e2e tests for auth, richer model turns, shell/git tool turns, terminal behavior, the browser tool adapter protocol, and later MCP/plugin adapters. | [#117](https://github.com/VertoOS/wasm-host/issues/117), [#3](https://github.com/VertoOS/wasm-host/issues/3) |
...
| M6: shell and git turn | Codex can run sandboxed shell/git/tool packages with cwd, env, stdio, timeout, cancellation, and child packaged-command invocation. The current command catalog proves loaded command discovery and explicit PATH lookup, the deterministic packaged tool fixture proves cwd/env/stdin/EOF/workspace-read plumbing, and the raw WASI worker has an internal child-command RPC plus `wasix_32v1.proc_exec`/`proc_exec2`/`proc_exec3` for process-replacement style packaged commands, asyncify vfork child completion, a serialized copied-memory child-exec subset, and package-root command shims that make loaded commands visible to Bash PATH lookup. The first non-interactive Bash/coreutils WebC smoke passes in real-browser e2e. Real git, process spawn, general blocking join, full fork/store cloning, broad shell semantics, interactive TTY, and native process spawning remain later. | Packaged tool fixture plus terminal transcript, raw WASI worker bridge, WASIX exec/fork assertions, and the passing Bash/coreutils browser e2e. | [#212](https://github.com/VertoOS/wasm-host/issues/212), [#211](https://github.com/VertoOS/wasm-host/issues/211), [#209](https://github.com/VertoOS/wasm-host/issues/209), [#181](https://github.com/VertoOS/wasm-host/issues/181), [#179](https://github.com/VertoOS/wasm-host/issues/179), [#177](https://github.com/VertoOS/wasm-host/issues/177), [#175](https://github.com/VertoOS/wasm-host/issues/175), [#114](https://github.com/VertoOS/wasm-host/issues/114), [#6](https://github.com/VertoOS/wasm-host/issues/6) |
