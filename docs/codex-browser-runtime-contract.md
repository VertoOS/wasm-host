...
- A package loader boundary for explicit bytes, Fetch-backed artifacts, WebC or
  ...
  derivation.
- A browser command catalog on top of loaded package records, with
  `command.catalog` introspection and explicit `packageId: null` PATH lookup
  over `/bin` and `/usr/bin` command aliases.
- A first WebC/WASIX executor boundary that routes loaded WebC packages to a
  dedicated browser runtime layer and delegates WASI-runner atoms through the
  raw Preview1 executor with read-only package-root files from WebC volumes and
  generated package-catalog command shims for Bash-style PATH probes.
...
- WebC/WASIX command runs can bridge the host-owned workspace through cloneable
  snapshots, including child packaged-command results from `proc_exec`, so
  Bash/coreutils can create, read, list, redirect, and remove files under
  `/workspace` in a real browser worker without passing live store instances to
  workers.
...
| Package imports | Declare required WASI/WASIX/component imports. | Instantiate supported imports and fail unsupported imports with classified capability errors. | [#183](https://github.com/VertoOS/wasm-host/issues/183), [#110](https://github.com/VertoOS/wasm-host/issues/110), [#3](https://github.com/VertoOS/wasm-host/issues/3) |
...
| Artifact shape | Interim raw WASI manifest exists for the version smoke; Codex also has a `wasm32-unknown-unknown` request-builder bundle. Loaded WebC packages now parse v2/v3 manifest metadata, expose package identity, commands, WASI hints, filesystem mappings, atom artifacts, and volume file spans, run WASI-runner atom bytes through the browser Preview1 executor, mount read-only WebC volume files as package-root files, project loaded command-catalog paths into package-root shims for guest PATH probes, and publish loaded commands through the browser command catalog/PATH resolver. Non-WASI runners and missing atoms report structured errors; full WASIX process spawning remains later work. | Publish browser-runnable Codex artifact contracts, including the near-term request-builder bundle and the later full-runtime package with manifest metadata, exports/imports, commands, hashes, and capability declarations. | [#181](https://github.com/VertoOS/wasm-host/issues/181), [#175](https://github.com/VertoOS/wasm-host/issues/175), [#172](https://github.com/VertoOS/wasm-host/issues/172), [#170](https://github.com/VertoOS/wasm-host/issues/170), [#168](https://github.com/VertoOS/wasm-host/issues/168), [#166](https://github.com/VertoOS/wasm-host/issues/166), [#164](https://github.com/VertoOS/wasm-host/issues/164), [#122](https://github.com/VertoOS/wasm-host/issues/122), [#109](https://github.com/VertoOS/wasm-host/issues/109) |
| WASI/WASIX imports | Browser raw WASI runner wires the tracked Preview1 smoke imports, mirrors supported Preview1 calls through `wasix_32v1`, exposes narrow `wasix_32v1.proc_exec`/`proc_exec2`/`proc_exec3` adapters that replace the current module with a cataloged packaged command through the child-command bridge with env/PATH overlays, supports `proc_exit2`, `proc_parent`, `proc_snapshot`, no-child and completed-child `proc_join`, bounded asyncify `proc_fork(copy_memory=false)` vfork and `proc_fork(copy_memory=true)` copied-child subsets, single-instance asyncify `stack_checkpoint`/`stack_restore` for modules with exported stack bounds or a host-owned fallback buffer, `getcwd`/`chdir`, package-root absolute path lookup plus virtual `/workspace` and `/tmp` directories under the root preopen, writable workspace snapshot import/export for WebC/WASIX child commands, `/workspace` and `/workspace%` mounted path spelling for writable requests, package-root cwd fallback for relative workspace operations after `cd /workspace`, `path_open2`, fd extension flags, fd duplication, workspace-backed stdio renumbering for shell redirects, in-memory pipes, `getpid`, `thread_id`, `thread_parallelism`, zero-duration `thread_sleep`, empty signal-disposition queries, imported `env.memory`, a deterministic negative-`NOTSUP` `wasi.thread-spawn` import shape, and non-interactive `tty_get`/`tty_set` WASIX state with host-supplied terminal columns/rows. In the fork subsets the child resumes with pid `0`, finishes via `proc_exit2` or the existing `proc_exec*` child-command bridge, and then the parent resumes with the child pid and can reap `JoinStatus::ExitNormal` using the Wasmer ABI `u16` normal exit code union offset; copied children run in a serialized fresh instance with copied linear memory, exported mutable globals, and inherited pipe-backed fd state, not a full store/global clone. It recognizes common `wasix_32v1` raw socket/network, inherited Preview1-style socket stubs, futex/event/process/dynamic/clock utility imports that return deterministic unsupported browser capability errors, treats `callback_signal` as a diagnostic no-op, and raw WASI runs can opt into grouped `diagnostics.unsupportedWasixCalls` results for smoke classification with child `proc_exec` diagnostics merged into parent results. The Bash/coreutils WebC import audit is captured in [docs/bash-coreutils-webc-import-audit.md](bash-coreutils-webc-import-audit.md), and the browser e2e now passes Bash/coreutils path-command, workspace file, persisted workspace script, command-substitution, pipeline/read, and status/stderr workflows through pinned WebC artifacts. A separate `packages/wasmer-sdk-adapter` spike maps wasm-host command requests to `@wasmer/sdk` primitives outside `apps/web`; it is not wired as the browser runtime backend yet. `proc_spawn`, full copied process/store semantics, general blocking join, signal, broad sockets, event loops, context switching, interactive TTY, dynamic linking, nonzero thread sleep, and threads are not general WASIX runtime support. | Inventory full Codex import requirements and implement or explicitly reject each unsupported import with stable errors. | [#226](https://github.com/VertoOS/wasm-host/issues/226), [#225](https://github.com/VertoOS/wasm-host/issues/225), [#223](https://github.com/VertoOS/wasm-host/issues/223), [#220](https://github.com/VertoOS/wasm-host/issues/220), [#217](https://github.com/VertoOS/wasm-host/issues/217), [#215](https://github.com/VertoOS/wasm-host/issues/215), [#212](https://github.com/VertoOS/wasm-host/issues/212), [#211](https://github.com/VertoOS/wasm-host/issues/211), [#209](https://github.com/VertoOS/wasm-host/issues/209), [#206](https://github.com/VertoOS/wasm-host/issues/206), [#198](https://github.com/VertoOS/wasm-host/issues/198), [#197](https://github.com/VertoOS/wasm-host/issues/197), [#196](https://github.com/VertoOS/wasm-host/issues/196), [#195](https://github.com/VertoOS/wasm-host/issues/195), [#194](https://github.com/VertoOS/wasm-host/issues/194), [#193](https://github.com/VertoOS/wasm-host/issues/193), [#190](https://github.com/VertoOS/wasm-host/issues/190), [#189](https://github.com/VertoOS/wasm-host/issues/189), [#187](https://github.com/VertoOS/wasm-host/issues/187), [#185](https://github.com/VertoOS/wasm-host/issues/185), [#183](https://github.com/VertoOS/wasm-host/issues/183), [#181](https://github.com/VertoOS/wasm-host/issues/181), [#110](https://github.com/VertoOS/wasm-host/issues/110), [#3](https://github.com/VertoOS/wasm-host/issues/3) |
...
| Workspace | Raw version smoke has read-only package files and volatile `/tmp`. The browser workspace store supports in-memory operations, whole-snapshot IndexedDB persistence, injected raw WASI `/workspace` fixture mounts, deterministic snapshot export/import for raw worker runs, WebC/WASIX child-command snapshot handoff, and a deterministic `codex-browser workspace-edit` fixture. Bash/coreutils e2e now proves workspace create/read/list/redirect/remove behavior through packaged commands, but the store is not wired into the app-server path or full Codex edit turns yet. | Persistent browser workspace with clear mount semantics, snapshots, import/export, and file edit turn coverage. | [#215](https://github.com/VertoOS/wasm-host/issues/215), [#113](https://github.com/VertoOS/wasm-host/issues/113), [#1](https://github.com/VertoOS/wasm-host/issues/1) |
| Shell, git, and tools | Native runner has explicit host-command bridges for development. The browser command worker can catalog loaded package commands, resolve explicit `packageId: null` runs through PATH, invoke loaded child packaged commands with cwd/env/stdin/stdout/stderr/timeout propagation, route raw WASI worker child-command requests through an internal RPC, support `wasix_32v1.proc_exec`/`proc_exec2`/`proc_exec3` for packaged command replacement with env/PATH overlays, project catalog command shims for guest shell PATH probes, carry workspace snapshots across WebC/WASIX child commands, and run deterministic packaged tool fixtures with terminal transcript capture. The first non-interactive Bash/coreutils WebC smoke now passes in a real browser, including workspace file, persisted script, command-substitution, pipeline/read, and status/stderr workflows. A separate Wasmer SDK adapter package now proves the contract mapping for package/command args, env/cwd, stdin/stdout/stderr, exit status, and workspace snapshot mounts without replacing the command worker. Real git, broad shell behavior, process spawn, general blocking join, full fork/store cloning, and native process spawning remain later. | Codex tool calls run sandboxed packaged commands with stdio, cwd, env, timeout, cancellation, workspace effects, and child command semantics. | [#226](https://github.com/VertoOS/wasm-host/issues/226), [#225](https://github.com/VertoOS/wasm-host/issues/225), [#223](https://github.com/VertoOS/wasm-host/issues/223), [#220](https://github.com/VertoOS/wasm-host/issues/220), [#219](https://github.com/VertoOS/wasm-host/issues/219), [#215](https://github.com/VertoOS/wasm-host/issues/215), [#212](https://github.com/VertoOS/wasm-host/issues/212), [#181](https://github.com/VertoOS/wasm-host/issues/181), [#179](https://github.com/VertoOS/wasm-host/issues/179), [#177](https://github.com/VertoOS/wasm-host/issues/177), [#175](https://github.com/VertoOS/wasm-host/issues/175), [#114](https://github.com/VertoOS/wasm-host/issues/114), [#6](https://github.com/VertoOS/wasm-host/issues/6) |
| Terminal and PTY | Browser terminal shell supports typed/pasted stdin, EOF, resize, cancel, and text output. Raw WASIX `tty_get` now sees host-supplied terminal columns/rows, but the runtime is not readline-grade PTY behavior. | Terminal contract supports interactive Codex sessions, readline behavior, resize, cancellation, and clean stream close semantics. | [#217](https://github.com/VertoOS/wasm-host/issues/217), [#115](https://github.com/VertoOS/wasm-host/issues/115), [#6](https://github.com/VertoOS/wasm-host/issues/6) |
...
| End-to-end parity | Current e2e proves `codex --version`, request building, a mocked model turn, workspace edit persistence, a browser app-server protocol fixture, a packaged tool fixture through the terminal transcript path, and Bash/coreutils WebC stages for path-command, workspace file behavior, persisted workspace script execution, Bash command substitution from builtin plus packaged child-command output, pipeline/read behavior, and status/stderr behavior. The page-level smoke result reports these as named stages so partial browser support is visible in CI. | Add staged Codex browser e2e tests for auth, richer model turns, shell/git tool turns, terminal behavior, the browser tool adapter protocol, and later MCP/plugin adapters. | [#225](https://github.com/VertoOS/wasm-host/issues/225), [#223](https://github.com/VertoOS/wasm-host/issues/223), [#220](https://github.com/VertoOS/wasm-host/issues/220), [#219](https://github.com/VertoOS/wasm-host/issues/219), [#215](https://github.com/VertoOS/wasm-host/issues/215), [#117](https://github.com/VertoOS/wasm-host/issues/117), [#3](https://github.com/VertoOS/wasm-host/issues/3) |
...
| Command-like tools | Stdio, cwd, env, EOF, resize, timeout, child packaged-command invocation, and transcript behavior use the packaged command and terminal protocols already exercised by the web tests. |
...
| M2: full artifact admission | A full Codex package manifest can be fetched or uploaded, validated, and either loaded or rejected with a complete missing-capability report. | Package-loader test plus conformance case for classified unsupported capabilities. | [#183](https://github.com/VertoOS/wasm-host/issues/183), [#109](https://github.com/VertoOS/wasm-host/issues/109), [#110](https://github.com/VertoOS/wasm-host/issues/110), [#3](https://github.com/VertoOS/wasm-host/issues/3) |
...
| M6: shell and git turn | Codex can run sandboxed shell/git/tool packages with cwd, env, stdio, timeout, cancellation, workspace effects, and child packaged-command invocation. The current command catalog proves loaded command discovery and explicit PATH lookup, the deterministic packaged tool fixture proves cwd/env/stdin/EOF/workspace-read plumbing, and the raw WASI worker has an internal child-command RPC plus `wasix_32v1.proc_exec`/`proc_exec2`/`proc_exec3` for process-replacement style packaged commands, asyncify vfork child completion, a serialized copied-memory child-exec subset with inherited pipe-backed fds, package-root command shims that make loaded commands visible to Bash PATH lookup, command-substitution capture for builtin and packaged child output, pipe/file-backed fd `0` capture for packaged child stdin, in-memory pipe flow across builtin and packaged commands, redirected Bash `read`, nonzero packaged child status propagation, redirected packaged child stderr capture, and workspace snapshot handoff for child command mutations. The Bash/coreutils WebC e2e now includes path-command, workspace file, persisted workspace script, command-substitution, pipeline/read, and status/stderr coverage. The Wasmer SDK adapter spike covers the same executor request/result fields and a real-browser COOP/COEP SDK import proof, but SDK package execution currently exits `45` with empty stdout/stderr for registry `wasmer/coreutils@1.0.25` `echo` and pinned `wasmer/bash@1.0.25` WebC bytes through `Wasmer.fromFile()`. Real git, process spawn, general blocking join, full fork/store cloning, broad shell semantics, interactive TTY, native process spawning, and Wasmer SDK exit-45 resolution remain later. | Packaged tool fixture plus terminal transcript, raw WASI worker bridge, WASIX exec/fork assertions, the passing Bash/coreutils browser e2e path/workspace/script/substitution/pipeline/status-stderr coverage, isolated Wasmer SDK adapter unit tests, and the Wasmer SDK browser parity blocker smoke. | [#230](https://github.com/VertoOS/wasm-host/issues/230), [#228](https://github.com/VertoOS/wasm-host/issues/228), [#226](https://github.com/VertoOS/wasm-host/issues/226), [#225](https://github.com/VertoOS/wasm-host/issues/225), [#223](https://github.com/VertoOS/wasm-host/issues/223), [#220](https://github.com/VertoOS/wasm-host/issues/220), [#219](https://github.com/VertoOS/wasm-host/issues/219), [#215](https://github.com/VertoOS/wasm-host/issues/215), [#212](https://github.com/VertoOS/wasm-host/issues/212), [#211](https://github.com/VertoOS/wasm-host/issues/211), [#209](https://github.com/VertoOS/wasm-host/issues/209), [#181](https://github.com/VertoOS/wasm-host/issues/181), [#179](https://github.com/VertoOS/wasm-host/issues/179), [#177](https://github.com/VertoOS/wasm-host/issues/177), [#175](https://github.com/VertoOS/wasm-host/issues/175), [#114](https://github.com/VertoOS/wasm-host/issues/114), [#6](https://github.com/VertoOS/wasm-host/issues/6) |
