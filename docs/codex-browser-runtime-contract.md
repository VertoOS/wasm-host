...
| Shell, git, and tools | Native runner has explicit host-command bridges for development. The browser command worker can catalog loaded package commands, resolve explicit `packageId: null` runs through PATH, and run a deterministic packaged tool fixture with cwd, filtered env, stdin/EOF, timeout, workspace reads, and terminal transcript capture, but not real Bash or git packages yet. | Codex tool calls run sandboxed packaged commands with stdio, cwd, env, timeout, and cancellation semantics. | [#175](https://github.com/VertoOS/wasm-host/issues/175), [#114](https://github.com/VertoOS/wasm-host/issues/114), [#6](https://github.com/VertoOS/wasm-host/issues/6) |
...
| M6: shell and git turn | Codex can run sandboxed shell/git/tool packages with cwd, env, stdio, timeout, and cancellation. The current command catalog proves loaded command discovery and explicit PATH lookup, and the deterministic packaged tool fixture proves cwd/env/stdin/EOF/workspace-read plumbing and terminal transcript capture while real Bash and git packages remain later. | Packaged tool fixture plus terminal transcript assertions. | [#175](https://github.com/VertoOS/wasm-host/issues/175), [#114](https://github.com/VertoOS/wasm-host/issues/114), [#6](https://github.com/VertoOS/wasm-host/issues/6) |
