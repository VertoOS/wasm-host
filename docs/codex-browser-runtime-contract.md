...
| M6: shell and git turn | Codex can run sandboxed shell/git/tool packages with cwd, env, stdio, timeout, cancellation, and child packaged-command invocation. The current command catalog proves loaded command discovery and explicit PATH lookup, the deterministic packaged tool fixture proves cwd/env/stdin/EOF/workspace-read plumbing, and the raw WASI worker has an internal child-command RPC plus `wasix_32v1.proc_exec` for process-replacement style packaged commands. Real Bash, git, process spawn/join, and native process spawning remain later. | Packaged tool fixture plus terminal transcript, raw WASI worker bridge, and WASIX `proc_exec` assertions. | [#181](https://github.com/VertoOS/wasm-host/issues/181), [#179](https://github.com/VertoOS/wasm-host/issues/179), [#177](https://github.com/VertoOS/wasm-host/issues/177), [#175](https://github.com/VertoOS/wasm-host/issues/175), [#114](https://github.com/VertoOS/wasm-host/issues/114), [#6](https://github.com/VertoOS/wasm-host/issues/6) |
