...
| Shell, git, and tools | Native runner has explicit host-command bridges for development. The browser command worker can catalog loaded package commands, resolve explicit `packageId: null` runs through PATH, invoke loaded child packaged commands with cwd/env/stdin/stdout/stderr/timeout propagation, route raw WASI worker child-command requests through an internal RPC, support `wasix_32v1.proc_exec`/`proc_exec2`/`proc_exec3` for packaged command replacement with env/PATH overlays, project catalog command shims for guest shell PATH probes, carry workspace snapshots across WebC/WASIX child commands, and run deterministic packaged tool fixtures with terminal transcript capture. The first non-interactive Bash/coreutils WebC smoke now passes in a real browser, including workspace file, persisted script, command-substitution, and pipeline/read workflows; real git, broad shell behavior, process spawn, general blocking join, full fork/store cloning, and native process spawning remain later. | Codex tool calls run sandboxed packaged commands with stdio, cwd, env, timeout, cancellation, workspace effects, and child command semantics. | [#223](https://github.com/VertoOS/wasm-host/issues/223), [#220](https://github.com/VertoOS/wasm-host/issues/220), [#219](https://github.com/VertoOS/wasm-host/issues/219), [#215](https://github.com/VertoOS/wasm-host/issues/215), [#212](https://github.com/VertoOS/wasm-host/issues/212), [#181](https://github.com/VertoOS/wasm-host/issues/181), [#179](https://github.com/VertoOS/wasm-host/issues/179), [#177](https://github.com/VertoOS/wasm-host/issues/177), [#175](https://github.com/VertoOS/wasm-host/issues/175), [#114](https://github.com/VertoOS/wasm-host/issues/114), [#6](https://github.com/VertoOS/wasm-host/issues/6) |
...
| End-to-end parity | Current e2e proves `codex --version`, request building, a mocked model turn, workspace edit persistence, a browser app-server protocol fixture, a packaged tool fixture through the terminal transcript path, and Bash/coreutils WebC stages for path-command, workspace file behavior, persisted workspace script execution, Bash command substitution from builtin plus packaged child-command output, and pipeline/read behavior. The page-level smoke result reports these as named stages so partial browser support is visible in CI. | Add staged Codex browser e2e tests for auth, richer model turns, shell/git tool turns, terminal behavior, the browser tool adapter protocol, and later MCP/plugin adapters. | [#223](https://github.com/VertoOS/wasm-host/issues/223), [#220](https://github.com/VertoOS/wasm-host/issues/220), [#219](https://github.com/VertoOS/wasm-host/issues/219), [#215](https://github.com/VertoOS/wasm-host/issues/215), [#117](https://github.com/VertoOS/wasm-host/issues/117), [#3](https://github.com/VertoOS/wasm-host/issues/3) |
