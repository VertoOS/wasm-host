...
| WASI/WASIX imports | Browser raw WASI runner wires the tracked Preview1 smoke imports, mirrors supported Preview1 calls through `wasix_32v1`, exposes narrow `wasix_32v1.proc_exec`/`proc_exec2`/`proc_exec3` adapters that replace the current module with a cataloged packaged command through the child-command bridge with env/PATH overlays, supports `proc_exit2`, `proc_parent`, `proc_snapshot`, no-child `proc_join`, single-instance asyncify `stack_checkpoint`/`stack_restore` for modules with exported stack bounds, `getcwd`/`chdir`, `path_open2`, fd extension flags, fd duplication, in-memory pipes, `getpid`, `thread_id`, `thread_parallelism`, zero-duration `thread_sleep`, empty signal-disposition queries, imported `env.memory`, a deterministic negative-`NOTSUP` `wasi.thread-spawn` import shape, and non-interactive `tty_get`/`tty_set` WASIX state. It recognizes common `wasix_32v1` raw socket/network, inherited Preview1-style socket stubs, futex/event/process/dynamic/clock utility imports that return deterministic unsupported browser capability errors, treats `callback_signal` as a diagnostic no-op, and raw WASI runs can opt into grouped `diagnostics.unsupportedWasixCalls` results for smoke classification with child `proc_exec` diagnostics merged into parent results. The Bash/coreutils WebC import audit is captured in [docs/bash-coreutils-webc-import-audit.md](bash-coreutils-webc-import-audit.md). `proc_spawn`, blocking join, fork, signal, broad sockets, event loops, context switching, interactive TTY, dynamic linking, nonzero thread sleep, process-level stack/store rewind, and threads are not general WASIX runtime support. | Inventory full Codex import requirements and implement or explicitly reject each unsupported import with stable errors. | [#206](https://github.com/VertoOS/wasm-host/issues/206), [#198](https://github.com/VertoOS/wasm-host/issues/198), [#197](https://github.com/VertoOS/wasm-host/issues/197), [#196](https://github.com/VertoOS/wasm-host/issues/196), [#195](https://github.com/VertoOS/wasm-host/issues/195), [#194](https://github.com/VertoOS/wasm-host/issues/194), [#193](https://github.com/VertoOS/wasm-host/issues/193), [#190](https://github.com/VertoOS/wasm-host/issues/190), [#189](https://github.com/VertoOS/wasm-host/issues/189), [#187](https://github.com/VertoOS/wasm-host/issues/187), [#185](https://github.com/VertoOS/wasm-host/issues/185), [#183](https://github.com/VertoOS/wasm-host/issues/183), [#181](https://github.com/VertoOS/wasm-host/issues/181), [#110](https://github.com/VertoOS/wasm-host/issues/110), [#3](https://github.com/VertoOS/wasm-host/issues/3) |
...
| M6: shell and git turn | Codex can run sandboxed shell/git/tool packages with cwd, env, stdio, timeout, cancellation, and child packaged-command invocation. The current command catalog proves loaded command discovery and explicit PATH lookup, the deterministic packaged tool fixture proves cwd/env/stdin/EOF/workspace-read plumbing, and the raw WASI worker has an internal child-command RPC plus `wasix_32v1.proc_exec`/`proc_exec2`/`proc_exec3` for process-replacement style packaged commands. Real Bash, git, process spawn/fork/blocking-join, and native process spawning remain later. | Packaged tool fixture plus terminal transcript, raw WASI worker bridge, and WASIX exec assertions. | [#181](https://github.com/VertoOS/wasm-host/issues/181), [#179](https://github.com/VertoOS/wasm-host/issues/179), [#177](https://github.com/VertoOS/wasm-host/issues/177), [#175](https://github.com/VertoOS/wasm-host/issues/175), [#114](https://github.com/VertoOS/wasm-host/issues/114), [#6](https://github.com/VertoOS/wasm-host/issues/6) |
