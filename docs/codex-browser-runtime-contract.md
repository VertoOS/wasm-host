...
| WASI/WASIX imports | Browser raw WASI runner wires the tracked Preview1 smoke imports, a narrow `wasix_32v1.proc_exec` adapter that replaces the current module with a cataloged packaged command through the child-command bridge, and common `wasix_32v1` raw socket/network imports that return deterministic `NOTSUP` browser capability errors. `proc_spawn`, join, fork, signal, broad sockets, and threads are not general WASIX runtime support. | Inventory full Codex import requirements and implement or explicitly reject each unsupported import with stable errors. | [#183](https://github.com/VertoOS/wasm-host/issues/183), [#181](https://github.com/VertoOS/wasm-host/issues/181), [#110](https://github.com/VertoOS/wasm-host/issues/110), [#3](https://github.com/VertoOS/wasm-host/issues/3) |
