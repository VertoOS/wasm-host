...
| WASI/WASIX imports | Browser raw WASI runner wires the tracked Preview1 smoke imports and a narrow `wasix_32v1.proc_exec` adapter that replaces the current module with a cataloged packaged command through the child-command bridge. `proc_spawn`, join, fork, signal, broad sockets, and threads are not general WASIX runtime support. | Inventory full Codex import requirements and implement or explicitly reject each unsupported import with stable errors. | [#181](https://github.com/VertoOS/wasm-host/issues/181), [#110](https://github.com/VertoOS/wasm-host/issues/110), [#3](https://github.com/VertoOS/wasm-host/issues/3) |
