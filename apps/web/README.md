...
- `src/wasi-module.js` implements the narrow raw WASI preview1 fixture runner
  ...
  `fd_read`, `fd_seek`, `fd_tell`, and `fd_advise`, plus a volatile in-memory
  `/tmp` scratch preopen for create, `path_create_directory`, write, readback,
...
- `test/wasi-module.test.js` covers raw WASI module byte loading, argv/env
  ...
  seek/tell/advice behavior, descriptor renumbering, volatile scratch-file
  allocation/truncate/sync and directory create/remove operations,
