...
| MCP and plugins | The browser adapter has a deterministic loopback MCP/tool fixture that can list and call a browser-owned tool, enforce bounded results, and reject native-only stdio/local-process server configs with structured capability errors. It is not the full MCP SDK, OAuth, plugin package runtime, or gateway-backed transport. | MCP/plugin servers run through explicit WASM, HTTP, WebSocket, or gateway transports with capability declarations. | [#116](https://github.com/VertoOS/wasm-host/issues/116) |
...
| End-to-end parity | Current e2e proves `codex --version`, request building, a mocked model turn, workspace edit persistence, a browser app-server protocol fixture, and a packaged tool fixture through the terminal transcript path. The page-level smoke result now reports these as named stages so partial browser support is visible in CI. | Add staged Codex browser e2e tests for auth, richer model turns, shell/git tool turns, terminal behavior, and MCP/plugin behavior. | [#117](https://github.com/VertoOS/wasm-host/issues/117), [#3](https://github.com/VertoOS/wasm-host/issues/3) |
...
| M8: MCP/plugin transport | Codex can discover and use a browser-safe MCP/plugin transport without native local process spawn. The current loopback fixture proves bounded browser-owned tool listing/calling and explicit unsupported stdio configs. | Fake MCP server over WASM, HTTP, WebSocket, or gateway fixture. | [#116](https://github.com/VertoOS/wasm-host/issues/116) |
