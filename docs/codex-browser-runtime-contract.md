...
| Auth and secrets | No browser-safe Codex credential provider is wired into `wasm-host`. The Codex demo can ask native `codex app-server` for device login. | Host-owned device login and secret provider with redaction, storage policy, cancellation, and no secret-bearing argv or logs. | [#111](https://github.com/VertoOS/wasm-host/issues/111) |
| Model HTTP | Browser HTTP bridge exists, but Codex model traffic is not integrated with it. The Codex demo can direct-fetch or use native app-server. | Route model traffic through host Fetch or gateway, with streaming events, cancellation, response limits, and secret-safe errors. | [#112](https://github.com/VertoOS/wasm-host/issues/112), [#7](https://github.com/VertoOS/wasm-host/issues/7) |
...
| M4: auth provider | Device login can start, complete or cancel, store a token through host-owned state, and avoid leaking secrets to argv, logs, manifests, or error messages. | Browser-safe auth provider test with fake device flow. | [#111](https://github.com/VertoOS/wasm-host/issues/111) |
