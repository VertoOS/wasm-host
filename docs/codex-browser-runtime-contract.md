...
- A first WebC/WASIX executor boundary that routes loaded WebC packages to a
  ...
  raw Preview1 executor with read-only package-root files from WebC volumes.
...
| Shell, git, and tools | Native runner has explicit host-command bridges for development. The browser command worker can catalog loaded package commands, resolve explicit `packageId: null` runs through PATH, invoke loaded child packaged commands with cwd/env/stdin/stdout/stderr/timeout propagation, route raw WASI worker child-command requests through an internal RPC, support `wasix_32v1.proc_exec`/`proc_exec2`/`proc_exec3` for packaged command replacement with env/PATH overlays, and run deterministic packaged tool fixtures with terminal transcript capture, but not real Bash or git packages yet. | Codex tool calls run sandboxed packaged commands with stdio, cwd, env, timeout, cancellation, and child command semantics. | [#181](https://github.com/VertoOS/wasm-host/issues/181), [#179](https://github.com/VertoOS/wasm-host/issues/179), [#177](https://github.com/VertoOS/wasm-host/issues/177), [#175](https://github.com/VertoOS/wasm-host/issues/175), [#114](https://github.com/VertoOS/wasm-host/issues/114), [#6](https://github.com/VertoOS/wasm-host/issues/6) |
