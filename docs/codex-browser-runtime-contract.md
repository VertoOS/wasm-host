...
| WASI/WASIX imports | Browser raw WASI runner wires the tracked Preview1 smoke imports, mirrors supported Preview1 calls through `wasix_32v1`, exposes narrow `wasix_32v1.proc_exec`/`proc_exec2`/`proc_exec3` adapters that replace the current module with a cataloged packaged command through the child-command bridge with env/PATH overlays, supports `proc_exit2`, `proc_parent`, `proc_snapshot`, `getcwd`/`chdir`, `path_open2`, fd extension flags, fd duplication, in-memory pipes, `getpid`, empty signal-disposition queries, imported `env.memory`, a deterministic negative-`NOTSUP` `wasi.thread-spawn` import shape, and non-interactive `tty_get`/`tty_set` WASIX state. It recognizes common `wasix_32v1` raw socket/network plus thread/futex/event/process/dynamic utility imports that return deterministic unsupported browser capability errors. The Bash/coreutils WebC import audit is captured in [docs/bash-coreutils-webc-import-audit.md](bash-coreutils-webc-import-audit.md); thread/event, dynamic, network, and clock behavior remain grouped runtime work. `proc_spawn`, join, fork, signal, broad sockets, event loops, context switching, interactive TTY, dynamic linking, and threads are not general WASIX runtime support. | Inventory full Codex import requirements and implement or explicitly reject each unsupported import with stable errors. | [#198](https://github.com/VertoOS/wasm-host/issues/198), [#197](https://github.com/VertoOS/wasm-host/issues/197), [#196](https://github.com/VertoOS/wasm-host/issues/196), [#195](https://github.com/VertoOS/wasm-host/issues/195), [#194](https://github.com/VertoOS/wasm-host/issues/194), [#193](https://github.com/VertoOS/wasm-host/issues/193), [#190](https://github.com/VertoOS/wasm-host/issues/190), [#189](https://github.com/VertoOS/wasm-host/issues/189), [#187](https://github.com/VertoOS/wasm-host/issues/187), [#185](https://github.com/VertoOS/wasm-host/issues/185), [#183](https://github.com/VertoOS/wasm-host/issues/183), [#181](https://github.com/VertoOS/wasm-host/issues/181), [#110](https://github.com/VertoOS/wasm-host/issues/110), [#3](https://github.com/VertoOS/wasm-host/issues/3) |
