...
| Plugins and MCP | Treat each plugin or MCP server as an explicit transport-backed capability. | Provide browser-safe WASM, HTTP, WebSocket, or gateway transports. | [#116](https://github.com/VertoOS/wasm-host/issues/116) |
...
| Auth and secrets | A minimal host-owned bearer secret seam can inject a token into mocked `codex-browser model-request` dispatch from an opaque `CODEX_MODEL_BEARER_SECRET_REF`. Full browser device-flow login, refresh, logout, persistence, and storage policy are not wired yet. | Host-owned device login and secret provider with redaction, storage policy, cancellation, and no secret-bearing argv or logs. | [#111](https://github.com/VertoOS/wasm-host/issues/111) |
| Model HTTP | The `codex-browser model-request` path can send a mocked model request through the host HTTP bridge and stream local fixture responses. Real provider traffic and SSE semantics are not integrated yet. | Route model traffic through host Fetch or gateway, with streaming events, cancellation, response limits, and secret-safe errors. | [#112](https://github.com/VertoOS/wasm-host/issues/112), [#7](https://github.com/VertoOS/wasm-host/issues/7) |
| Workspace | Raw version smoke has read-only package files and volatile `/tmp`. | Persistent browser workspace with clear mount semantics, snapshots, import/export, and file edit turn coverage. | [#113](https://github.com/VertoOS/wasm-host/issues/113), [#1](https://github.com/VertoOS/wasm-host/issues/1) |
| Shell, git, and tools | Native runner has explicit host-command bridges for development; browser app does not run packaged shell/git tools yet. | Codex tool calls run sandboxed packaged commands with stdio, cwd, env, timeout, and cancellation semantics. | [#114](https://github.com/VertoOS/wasm-host/issues/114), [#6](https://github.com/VertoOS/wasm-host/issues/6) |
...
| MCP and plugins | No Codex MCP/plugin transport is wired into the browser runtime. | MCP/plugin servers run through explicit WASM, HTTP, WebSocket, or gateway transports with capability declarations. | [#116](https://github.com/VertoOS/wasm-host/issues/116) |
| Browser app-server protocol | Codex browser demo speaks app-server JSON-RPC over WebSocket, but its server spawns native `codex app-server`. | Browser-hosted app-server/session runtime that does not require a native child process. | [#118](https://github.com/VertoOS/wasm-host/issues/118) |
| End-to-end parity | Current e2e proves only `codex --version`. | Add staged Codex browser e2e tests for request build, auth, one model turn, file edit, shell/git tool turn, terminal, and MCP/plugin behavior. | [#117](https://github.com/VertoOS/wasm-host/issues/117), [#3](https://github.com/VertoOS/wasm-host/issues/3) |
...
| M3: model turn without credentials | A Codex turn can send a mocked model request through the host HTTP/gateway bridge and consume a streamed response. | Local gateway fixture and browser worker test, no external network. | [#112](https://github.com/VertoOS/wasm-host/issues/112), [#117](https://github.com/VertoOS/wasm-host/issues/117) |
| M4: auth provider | Device login can start, complete or cancel, store a token through host-owned state, and avoid leaking secrets to argv, logs, manifests, or error messages. The current bearer-secret seam is only the first dispatch-time injection layer. | Browser-safe auth provider test with fake device flow. | [#111](https://github.com/VertoOS/wasm-host/issues/111) |
| M5: file edit turn | Codex can read a workspace file, produce an edit, write it back, and persist or export the workspace. | Browser workspace fixture and e2e file edit scenario. | [#113](https://github.com/VertoOS/wasm-host/issues/113), [#117](https://github.com/VertoOS/wasm-host/issues/117) |
| M6: shell and git turn | Codex can run sandboxed shell/git/tool packages with cwd, env, stdio, timeout, and cancellation. | Packaged tool fixture plus terminal transcript assertions. | [#114](https://github.com/VertoOS/wasm-host/issues/114), [#6](https://github.com/VertoOS/wasm-host/issues/6) |
...
| M8: MCP/plugin transport | Codex can discover and use a browser-safe MCP/plugin transport without native local process spawn. | Fake MCP server over WASM, HTTP, WebSocket, or gateway fixture. | [#116](https://github.com/VertoOS/wasm-host/issues/116) |
| M9: browser-hosted app-server | The app-server/session protocol runs through browser-hosted services instead of spawning native `codex app-server`. | Browser e2e using app-server JSON-RPC over the host runtime. | [#118](https://github.com/VertoOS/wasm-host/issues/118), [#117](https://github.com/VertoOS/wasm-host/issues/117) |
