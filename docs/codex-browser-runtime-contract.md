...
| Tool adapters, plugins, and MCP | The browser adapter does not expose MCP as a first-class runtime module. Current proofs are the packaged tool fixture, terminal transcript plumbing, and app-server session protocol; the next layer is a browser tool protocol with discovery, invocation, result, approval, cancellation, and workspace-effect semantics that MCP/plugin adapters can translate into. | MCP/plugin servers run through explicit packaged command, WASM, HTTP, WebSocket, or gateway adapters with capability declarations. Native stdio/local process launch stays unsupported in browser hosts. | [#156](https://github.com/VertoOS/wasm-host/issues/156), [#116](https://github.com/VertoOS/wasm-host/issues/116) |
...
the protocol itself. The browser host needs these neutral concepts first:
...
| M8: browser tool adapter protocol | Codex can discover and call browser-safe tools through a protocol-neutral adapter boundary without native local process spawn. Tool specs, calls, results, approval/cancellation state, and workspace effects are explicit enough for MCP/plugin adapters to layer on later. | Fake tool adapter over packaged command, WASM, HTTP, WebSocket, or gateway fixture. | [#156](https://github.com/VertoOS/wasm-host/issues/156), [#116](https://github.com/VertoOS/wasm-host/issues/116) |
