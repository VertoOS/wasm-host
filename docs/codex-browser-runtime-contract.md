...
- A deterministic `browser-tool-fixture` package type can run a packaged
  ...
  through the command worker and real-browser e2e harness.
...
| Shell, git, and tools | Native runner has explicit host-command bridges for development. The browser command worker can run a deterministic packaged tool fixture with cwd, filtered env, stdin, timeout, and workspace reads, but not real Bash or git packages yet. | Codex tool calls run sandboxed packaged commands with stdio, cwd, env, timeout, and cancellation semantics. | [#114](https://github.com/VertoOS/wasm-host/issues/114), [#6](https://github.com/VertoOS/wasm-host/issues/6) |
...
| End-to-end parity | Current e2e proves `codex --version`, request building, a mocked model turn, workspace edit persistence, and a packaged tool fixture path. | Add staged Codex browser e2e tests for request build, auth, one model turn, file edit, shell/git tool turn, terminal, and MCP/plugin behavior. | [#117](https://github.com/VertoOS/wasm-host/issues/117), [#3](https://github.com/VertoOS/wasm-host/issues/3) |
...
| M6: shell and git turn | Codex can run sandboxed shell/git/tool packages with cwd, env, stdio, timeout, and cancellation. The current deterministic packaged tool fixture proves cwd/env/stdin/workspace-read plumbing while real Bash and git packages remain later. | Packaged tool fixture plus terminal transcript assertions. | [#114](https://github.com/VertoOS/wasm-host/issues/114), [#6](https://github.com/VertoOS/wasm-host/issues/6) |
