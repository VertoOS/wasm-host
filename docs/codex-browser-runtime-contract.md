...
| Shell, git, and tools | Native runner has explicit host-command bridges for development. The browser command worker can catalog loaded package commands, resolve explicit `packageId: null` runs through PATH, invoke loaded child packaged commands with cwd/env/stdin/stdout/stderr/timeout propagation, and run deterministic packaged tool fixtures with terminal transcript capture, but not real Bash or git packages yet. | Codex tool calls run sandboxed packaged commands with stdio, cwd, env, timeout, cancellation, and child command semantics. | [#177](https://github.com/VertoOS/wasm-host/issues/177), [#175](https://github.com/VertoOS/wasm-host/issues/175), [#114](https://github.com/VertoOS/wasm-host/issues/114), [#6](https://github.com/VertoOS/wasm-host/issues/6) |
...
| M6: shell and git turn | Codex can run sandboxed shell/git/tool packages with cwd, env, stdio, timeout, cancellation, and child packaged-command invocation. The current command catalog proves loaded command discovery and explicit PATH lookup, and the deterministic packaged tool fixture proves cwd/env/stdin/EOF/workspace-read plumbing, child command dispatch, and terminal transcript capture while real Bash and git packages remain later. This is still not native process spawn, and raw WASI worker-side WASIX process imports need a later worker-safe bridge. | Packaged tool fixture plus terminal transcript assertions. | [#177](https://github.com/VertoOS/wasm-host/issues/177), [#175](https://github.com/VertoOS/wasm-host/issues/175), [#114](https://github.com/VertoOS/wasm-host/issues/114), [#6](https://github.com/VertoOS/wasm-host/issues/6) |
